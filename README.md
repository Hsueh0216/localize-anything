...
<p align="center">
  ...
  <img alt="当前版本：v0.3.2" src="https://img.shields.io/badge/release-v0.3.2-blue" />
...
- 为 Android、iOS 或常见文本资源生成可审查的目标语言文件；
...
**当前公开版本：** [v0.3.2 — Android Coverage Diagnostics](https://github.com/xueyang-dev/localize-anything/releases/tag/v0.3.2)

v0.3.2 是一个诊断与文档版本：它新增 Android 覆盖范围诊断，用于区分 app 源码资源和 Gradle merged dependency resources，并在 source-only Android 本地化可能无法覆盖全部可见 UI 字符串时给出明确警告。它不包含 merged-resource overlay 生成，不包含 apply-to-project 写回，也不运行 provider-backed translation。
...
这些结果证明的是流程、结构保护和交付证据的正确性，不等同于人工译文质量评价，也不表示生成结果可以免审查直接上线。详细记录见 [变更记录](CHANGELOG.md)、[Android 覆盖范围模型](docs/android-coverage-model.md)、[v0.3.1 发布审计](docs/v0.3.1-release-audit.md) 和 [真实 Android 项目压力矩阵](docs/android-real-project-stress-matrix.md)。
...
| 平台资源 | 当前边界 |
| --- | --- |
| Android `strings.xml` | 支持 `string`、`string-array`、`plurals`，并进行暂存和确定性 QA |
