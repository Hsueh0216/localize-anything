...
<p align="center">
  ...
  <img alt="当前版本：v0.4.0" src="https://img.shields.io/badge/release-v0.4.0-blue" />
...
**当前公开版本：** [v0.4.0 — Word Document Localization](https://github.com/xueyang-dev/localize-anything/releases/tag/v0.4.0)

v0.4.0 新增 Word OpenXML 文档高保真本地化，支持 `.docx`、`.dotx`、`.docm` 和 `.dotm`，并在 Workbench 中加入文件/文件夹选择与拖拽导入入口。Word 输出会按目标语言规范化译文 run 的字体，例如英文使用 Arial、简体中文使用 Microsoft YaHei，同时由确定性 QA 保护包结构、relationships、styles、非文本资源、非字体 run 属性、段落属性、占位符和宏字节。

这个版本也加入显式 opt-in 的 Android merged dependency resource overlay 生成，用于需要本地化依赖资源的 Android 项目。旧二进制 `.doc`、图片文字、嵌入对象内容和 provider-backed translation 语义质量仍不属于确定性覆盖声明。
